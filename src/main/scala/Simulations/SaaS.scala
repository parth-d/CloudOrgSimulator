...
/**
 * This simulation is used to simulate a Software-as-a-Service Cloud
 *
...
object SaaS:
  ...
  private def createVm(simulation_number : Int) : Vm = {
    ...
    val vm_BW : Int     = config.getInt("SaaS.CloudProviderProperties.vm" + simulation_number + ".BandwidthInMBps")
    val vm_Size : Int   = config.getInt("SaaS.CloudProviderProperties.vm" + simulation_number + ".StorageInMBs")
  ...
  private def createCloudlets() : List[Cloudlet] = {
    ...
    val utilizationModel : UtilizationModelDynamic = new UtilizationModelDynamic(config.getDouble("SaaS.utilizationRatio")).setMaxResourceUtilization(0.1)
  ...
  private def createCloudlets(num_Cloudlets: Int, model: UtilizationModelDynamic, listbuffer: ListBuffer[Cloudlet]) : Unit = {
    ...
    val cloudletFileSize : Int  = config.getInt("SaaS.BrokerProperties.cloudlet.filesize")
